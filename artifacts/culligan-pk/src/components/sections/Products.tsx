          ...
          {bottles.map((bottle, i) => (
            ...
            >
              ...
              <div className="p-4 flex flex-col flex-1 gap-2">
                ...
                {/* Order CTA */}
                <motion.button
                  onClick={() => addItem({ id: bottle.id, name: bottle.name, label: bottle.label, price: bottle.price, unit: bottle.unit, image: bottle.image })}
                  className={`btn-cta-primary mt-2 w-full rounded-xl py-2.5 text-sm font-bold flex items-center justify-center gap-1.5
                    ${bottle.highlight
              ...
              {(() => {
                const monthlyLiters = people * litersPerDay * 30;
                let recSize = "19 Litre";
                let recBottles = Math.ceil(monthlyLiters / 19);
                let recPrice = 380;
                if (monthlyLiters <= 72) { recSize = "6 Litre"; recBottles = Math.ceil(monthlyLiters / 6); recPrice = 235; }
                else if (monthlyLiters <= 180) { recSize = "12 Litre"; recBottles = Math.ceil(monthlyLiters / 12); recPrice = 275; }
                const monthlyTotal = recBottles * recPrice;

                return (
                  <div className="mt-10">
                    {/* Heading */}
                    <div className="text-center mb-6">
                      <p className="text-[11px] font-extrabold tracking-[0.22em] uppercase mb-2" style={{ color: "#1d6fa4" }}>
                        Smart Estimator
                      </p>
                      ...
                      <p className="mt-2 text-sm text-slate-500 max-w-md mx-auto">
                        Tell us your household size and we'll recommend the right bottle size and how many to order each month.
                      </p>
                    </div>
                    ...
                    <motion.div
                      ...
                      style={{
                        ...
                        border: "1px solid rgba(147,210,250,0.35)",
                        boxShadow: "0 4px 32px rgba(33,150,243,0.1), 0 1px 6px rgba(33,150,243,0.06)",
                        backdropFilter: "blur(12px)",
                      }}
                    >
                      {/* Header */}
                      <div className="px-6 pt-6 pb-4 border-b" style={{ borderColor: "rgba(147,210,250,0.2)" }}>
                        <div className="flex items-center gap-2.5">
                          ...
                          <div>
                            <h3 className="font-black text-slate-900 text-[15px] leading-tight">Water Consumption Calculator</h3>
                            <p className="text-[11px] font-medium" style={{ color: "#64748b" }}>Estimate your monthly bottle needs</p>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="px-6 py-5 grid grid-cols-1 sm:grid-cols-2 gap-6">
                        {/* People stepper */}
                        <div>
                          <label className="text-[11.5px] font-bold uppercase tracking-[0.1em] mb-3 block" style={{ color: "#1d6fa4" }}>
                            Household Size
                          </label>
                          <div className="flex items-center gap-3">
                            <button
                              onClick={() => setPeople(p => Math.max(1, p - 1))}
                              className="w-9 h-9 rounded-xl font-bold text-lg flex items-center justify-center transition-all duration-150 active:scale-90 select-none"
                              style={{ background: "rgba(33,150,243,0.08)", color: "#2196f3", border: "1px solid rgba(33,150,243,0.2)" }}
                            >−</button>
                            <div className="flex-1 text-center">
                              <span className="text-3xl font-black text-slate-900">{people}</span>
                              <span className="text-sm text-slate-400 ml-1.5">people</span>
                            </div>
                            <button
                        ...
                        <div>
                          ...
                          <div className="flex items-center gap-2 flex-wrap">
                            {[1, 1.5, 2, 2.5, 3].map((v) => (
                              <button
                                key={v}
                                onClick={() => setLitersPerDay(v)}
                                className="px-3 py-1.5 rounded-lg text-[12px] font-bold transition-all duration-150"
                                style={litersPerDay === v ? {
                      ...
                      {/* Result */}
                      <motion.div
                        key={`${people}-${litersPerDay}`}
                        initial={{ opacity: 0, y: 6 }}
                        animate={{ opacity: 1, y: 0 }}
                        transition={{ duration: 0.25 }}
                        className="mx-6 mb-6 rounded-2xl px-5 py-4"
                        style={{
                          background: "linear-gradient(135deg, rgba(33,150,243,0.06) 0%, rgba(66,165,245,0.04) 100%)",
                          border: "1px solid rgba(66,165,245,0.2)",
                        }}
                      >
                        <div className="flex items-center justify-between flex-wrap gap-3">
                          <div>
                            <p className="text-[11px] font-bold uppercase tracking-[0.12em] mb-1" style={{ color: "#64748b" }}>
                              Monthly usage · {monthlyLiters.toFixed(0)}L
                            </p>
                            <p className="font-black text-slate-900" style={{ fontSize: "clamp(1.1rem, 3vw, 1.4rem)" }}>
                              <span style={{ color: "#2196f3" }}>{recBottles}×</span> {recSize} bottles
                            </p>
                            <p className="text-[12px] font-medium mt-0.5" style={{ color: "#64748b" }}>
                              ≈ <strong className="text-slate-800">Rs {monthlyTotal.toLocaleString()}</strong> / month
                            </p>
                          </div>
                          <a
                            href="#order"
                            className="btn-cta-primary inline-flex items-center gap-1.5 px-5 py-2.5 rounded-xl text-white font-bold text-[13px] shrink-0"
                            style={{
                              background: "linear-gradient(135deg,#2196f3,#42a5f5)",
                            }}
                          >
            ...
            <motion.div key="dispensers-grid" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -16 }} transition={{ duration: 0.25 }}>
              <div className="flex flex-wrap gap-5 justify-center">
                ...
                <div className="group w-full max-w-xs rounded-2xl bg-white border border-slate-100 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 overflow-hidden">
                  ...
                  <div className="p-5">
                    ...
                    <button
                      onClick={() => addItem({ id: "cradle-tap", name: "Cradle Tap Stand", label: "Gravity-fed tap", price: 1000, unit: "/ unit", image: "/cradle-tap-stand.jpg" })}
                      className="btn-cta-primary w-full flex items-center justify-center gap-2 rounded-xl py-2.5 text-sm font-bold text-white hover:brightness-105"
                ...
                <div className="group w-full max-w-xs rounded-2xl bg-white border border-slate-100 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 overflow-hidden">
                  ...
                  <div className="p-5">
                    ...
                    <button
                      onClick={() => addItem({ id: "homage-dispenser", name: "Homage Dispenser", label: "Hot & Cold, 3 Taps", price: 27500, unit: "/ unit", image: "/homage-dispenser.png" })}
                      className="btn-cta-primary w-full flex items-center justify-center gap-2 rounded-xl py-2.5 text-sm font-bold text-white hover:brightness-105"
                      style={{ background: "#42a5f5" }}
                    >
                      <ShoppingCart className="w-4 h-4" />
                ...
                <div className="group w-full max-w-xs rounded-2xl bg-white border border-slate-100 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 overflow-hidden">
                  ...
                  <div className="p-5">
                    ...
                    <button
                      onClick={() => addItem({ id: "tabletop-dispenser", name: "Tabletop Dispenser", label: "Room temp, compact", price: 950, unit: "/ unit", image: "/tabletop-dispenser.png" })}
                      className="btn-cta-primary w-full flex items-center justify-center gap-2 rounded-xl py-2.5 text-sm font-bold text-white hover:brightness-105"
                      style={{ background: "#42a5f5" }}
                    >
                      <ShoppingCart className="w-4 h-4" />
              ...
              <div className="text-center mt-8">
                <a href="/dispensers" className="inline-flex items-center gap-1.5 text-sm font-semibold text-[#42a5f5] hover:underline">
      ...
      <div className="relative py-20 overflow-hidden bg-white">
        ...
        <div className="container mx-auto px-4 md:px-6 relative z-10">
          <div className="text-center mb-12">
            ...
            >
              Why Customers Choose Culligan Water
            </motion.h3>
            <motion.p
              ...
              className="mt-4 text-slate-600 text-base leading-relaxed max-w-2xl mx-auto text-center"
            >
              When it comes to drinking water, customers want more than convenience. They want quality they can trust, reliable delivery, and a service that fits daily life. That's exactly what we aim to provide.
            </motion.p>
          </div>
            ...
            {whyPoints.map((pt, i) => {
              ...
              return (
                <motion.div
                  ...
                  whileHover={{ y: -4 }}
                  className="rounded-2xl p-6 bg-white/70 backdrop-blur-md border border-white/80 shadow-sm hover:bg-white hover:shadow-md transition-all duration-300"
                >
                  <div className="w-12 h-12 rounded-2xl bg-blue-100 border border-blue-200/60 flex items-center justify-center mb-4">
                    <Icon className="h-6 w-6 text-blue-600" />
                  </div>
                  <h4 className="font-bold text-slate-900 text-base mb-2">{pt.title}</h4>
            ...
            return (
              ...
              >
                ...
                <div className="p-4 flex flex-col flex-1 gap-2">
                  ...
                  <button
                    onClick={() => addItem({ id: bundle.id, name: bundle.name, label: bundle.contents, price: bundle.salePrice, unit: "/ bundle", image: bundle.image })}
                    className={`btn-cta-primary mt-2 w-full rounded-xl py-2.5 text-sm font-bold flex items-center justify-center gap-1.5
                      ${highlight
                        ? "bg-primary text-white hover:bg-primary/90"
                        : "bg-slate-900 text-white hover:bg-primary"

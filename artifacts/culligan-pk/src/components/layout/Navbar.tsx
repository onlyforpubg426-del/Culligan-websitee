...
const navGroups = [
  ...
  },
  {
    label: "Learn",
    links: [
      { name: "About Water",      href: "/about-water"  },
      { name: "How We Purify",    href: "#purification" },
    ],
  },
  {
    label: "Company",
    links: [
      ...
      { name: "Find Us Near You", href: "#outlets"      },
    ...
    <>
      ...
      >
        <div className="h-full max-w-[1440px] mx-auto px-4 sm:px-6 flex items-center justify-between">
          ...
          {/* Logo */}
          <Link href="/" className="flex items-center shrink-0 select-none" onClick={closeMenu}>
            <motion.img
              src={`${BASE}/culligan-logo.png`}
              alt="Culligan Water"
              animate={{ height: scrolled ? 34 : 42 }}
              transition={{ duration: 0.25 }}
              className="w-auto max-w-[160px] object-contain object-left drop-shadow-sm"
            />
          </Link>
          ...
          <nav className="hidden lg:flex items-center" aria-label="Primary">
            {allLinks.map((link) => {
              ...
              const cls = `relative px-3 py-2 text-[13px] font-semibold whitespace-nowrap rounded-lg
                           ...
                           ${isHovered ? "text-[#1d6fa4]" : "text-slate-600 hover:text-[#1d6fa4]"}`;
              const inner = (
                <>
                  {link.name}
                  {isHovered && (
          ...
          <div className="hidden lg:flex items-center gap-2">
            ...
            <a
              href="#order"
              className="btn-cta-primary inline-flex items-center gap-1.5
          ...
          <div className="lg:hidden flex items-center gap-1">
              ...
              <AnimatePresence>
                {totalItems > 0 && (
                  <motion.span
                    ...
                    exit={{ scale: 0 }}
                    transition={{ type: "spring", stiffness: 400, damping: 20 }}
                    className="absolute -top-0.5 -right-0.5 min-w-[16px] h-[16px]
            ...
            >
              <span className="flex flex-col justify-center items-center w-5 h-5 gap-[4.5px]">
                <motion.span
                  animate={menuOpen ? { rotate: 45, y: 6.5 } : { rotate: 0, y: 0 }}
                  transition={{ duration: 0.22, ease: [0.4, 0, 0.2, 1] as unknown as string }}
                  className="block w-5 h-[2px] rounded-full bg-current origin-center"
                />
                <motion.span
            ...
            >
                    ...
                    {group.links.map((link, li) => (
                      <motion.button
                        ...
                        className="w-full flex items-center justify-between
                                   ...
                                   hover:bg-blue-50/70 hover:text-[#1d6fa4]
                                   active:bg-blue-100
                                   transition-colors duration-150
                                   touch-manipulation text-left"
                      >
                        <span>{link.name}</span>
                        <ArrowRight className="w-4 h-4 opacity-25 shrink-0" />
                      </motion.button>
                    ))}
                  </div>
                ))}
              ...
              >
                <a
                  href="#order"
                  onClick={closeMenu}
                  className="btn-cta-whatsapp flex items-center justify-center gap-2

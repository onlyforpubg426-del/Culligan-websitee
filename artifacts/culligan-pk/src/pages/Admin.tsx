...
export default function Admin() {
  const [tab, setTab] = useState<"orders" | "enquiries" | "contact">("orders");
  ...
  const [newCounts, setNewCounts] = useState({ orders: 0, leads: 0, contact: 0 });
  ...
  useEffect(() => {
    ...
    fetchLeads();
    fetchContacts();
  }, [fetchOrders, fetchLeads, fetchContacts]);
  ...
  const handleTabClick = (key: "orders" | "enquiries" | "contact") => {
    setTab(key);
    setNewCounts((c) => ({
      ...c,
      [key === "enquiries" ? "leads" : key]: 0,
    }));
  };
                ...
                {leads.map((lead) => (
                  <div key={lead.id} className="bg-white rounded-2xl border border-slate-200 p-5">
                    <div className="flex items-start justify-between gap-4">
                      ...
                      <a
                        href={`tel:${lead.phone}`}
                        className="shrink-0 text-xs font-semibold px-3 py-1.5 rounded-lg bg-green-50 text-green-700 hover:bg-green-100 transition-colors"
                      >
                        📞 Call
                      </a>
                    </div>
